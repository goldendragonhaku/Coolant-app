...
with col_main:
    ...
    if m_choice != "+ New Machine":
        ...
        last_rec = c.fetchone()
        if last_rec: 
            last_vol, last_ri = last_rec[0], last_rec[1]

    # RI and Volume (with recall values)
    c1, c2 = st.columns(2)
...
with col_chart:
    ...
    if m_id and customer:
        hist_df = pd.read_sql_query(f"SELECT date, conc, ph FROM logs WHERE customer='{customer}' AND m_id='{m_id}' ORDER BY date ASC", conn)
        if not hist_df.empty:
            hist_df['date'] = pd.to_datetime(hist_df['date'])
            # Concentration Line Chart
            chart = alt.Chart(hist_df).mark_line(color=QC_GREEN, point=True).encode(
                x='date:T',
            ...
            st.info("Trend will appear after first saved entry.")

# --- 6. OBSERVATIONS ---
st.markdown("---")
st.text_area("Observations", value=st.session_state.master_notes, key="notes_widget", on_change=sync_from_widget, height=120)
...
if st.button("💾 SAVE MACHINE LOG", use_container_width=True):
    if customer and m_id:
        ...
        conn.commit()
        st.session_state.master_notes = ""
        st.success("Entry Saved to QualiServ Database.")
        st.rerun()

# --- 7. RECENT LOGS ---
st.markdown("### <span class='green-text'>Recent Session Entries</span>", unsafe_allow_html=True)
if customer:
    rec_df = pd.read_sql_query(f"SELECT date, m_id, conc, ph, notes FROM logs WHERE customer='{customer}' ORDER BY id DESC LIMIT 5", conn)
    if not rec_df.empty:
        st.table(rec_df)
